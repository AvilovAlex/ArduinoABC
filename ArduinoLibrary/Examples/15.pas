//Модификация игры в 15 с управлением Arduino-джойстиком HW-504
uses ArduinoABC, GraphABC, ABCObjects, ABCButtons;
...
var
  ...
  myUno: Arduino;
  ///Флаг, отвечающий за то, что джойстик был в позиции (0,0)
  wasInZeroPosition: boolean := true;
...
/// Проверка, что ход не приведет к выходу за пределы поля
function moveAvaliable(fx: integer; fy: integer): boolean;
begin
  result := (fx > 0) and (fx <= 4) and
            (fy > 0) and (fy <= 4);
end;
...
begin
  if EndOfGame then // Если все фишки на своих местах, то не реагировать на джойстик и ждать нажатия кнопки "Перемешать"
    exit;
  var delta := (512 - value) div 258;
  var fx := EmptyCellX;
  var fy := EmptyCellY;
  case pinNumber of
  ...
  if not (delta = 0) and wasInZeroPosition and moveAvaliable(fx, fy) then
  begin

//Модификация игры KillThem с управлением Arduino-джойстиком HW-504
uses ArduinoABC, ABCObjects, GraphABC, Timers;
...
const
  ...
  firePin = 0;
  ///Номер пина, к которому подключен выход нажатия джойстика HW-504
  pressPin = 2;
...
begin
  ...
  begin
    ...
    kSpaceKey := 0;
    //Выключаем индикатор успешного попадания
  ...
  begin
    ...
    ChangeInfoString;
    RedrawObjects;
    //Выключаем индикатор успешного попадания, если он до сих пор включен
    myUno.DigitalWrite(ledPin, false);
  end;
end;

//Модификация для работы с Arduino

/// Обработчик движения джойстика
procedure JoyStickMove(object: Arduino; pinNumber: integer; value: integer);
begin
  case pinNumber of
    movePin:
      begin
    ...
    firePin: kSpaceKey := value >= 550 ? 2 : 1;
  end;
end;

/// Обработчик нажатия на джойстик
procedure JoyStickPress(object: Arduino; pinNumber: integer; value: boolean);
begin
  if (pinNumber = pressPin) and value then
    KeyPress('G');
end;
...
begin
  ...
  myUno.SetPinMode(movePin, AnalogInput);
  myUno.SetPinMode(firePin, AnalogInput);
  myUno.SetPinMode(pressPin, DigitalInput);
  ...
  //Привязка обработчиков событий от джойстика
  myUno.AnalogPinChanged += JoyStickMove;
  myUno.DigitalPinChanged += JoyStickPress;
